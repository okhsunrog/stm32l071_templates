...
fn flash_range_from_linker() -> Range<u32> {
    ...
    assert!(
        linker_start >= FLASH_OFFSET,
        "Storage start symbol seems below flash base."
    );
    assert!(end > start, "Storage range invalid: end > start.");
    assert!(
        size >= MAX_ERASE_SIZE as u32,
        "Storage range must be >= MAX_ERASE_SIZE."
    );
    assert_eq!(
        size as usize % MAX_ERASE_SIZE,
        0,
        "Storage size must be multiple of MAX_ERASE_SIZE."
    );
    assert_eq!(
        start % MAX_ERASE_SIZE as u32,
        0,
        "Storage start must be MAX_ERASE_SIZE-aligned."
    );
    assert_eq!(
        end % MAX_ERASE_SIZE as u32,
        0,
        "Storage end must be MAX_ERASE_SIZE-aligned."
    );
    let calculated_pages = size as usize / MAX_ERASE_SIZE;
    assert_eq!(
        calculated_pages, PAGE_COUNT,
        "Calculated pages {} != PAGE_COUNT {}",
        calculated_pages, PAGE_COUNT
    );
    defmt::info!(
        "Storage: Linker symbols: start=0x{:X}, end=0x{:X}",
        linker_start,
        linker_end
    );
    defmt::info!(
        "Storage: Calculated HAL range: start=0x{:X}, end=0x{:X} ({} bytes, {} pages based on MAX_ERASE_SIZE={})",
        start,
        end,
        size,
        PAGE_COUNT,
...
pub fn init(flash: HalFlash) {
    ...
    let initial_state = StorageState {
        flash: wrapped_flash,
        cache:
            sequential_storage::cache::KeyPointerCache::<PAGE_COUNT, CacheKeyType, CACHE_KEYS>::new(
            ),
        flash_range,
    };
    ...
    match get::<u8>(MARKER_KEY) {
        Ok(Some(val)) if val == MARKER_VALUE => defmt::info!(
            "Storage: Found valid initialization marker (0x{:02X}).",
            val
        ),
        Ok(Some(val)) => {
            defmt::warn!(
                "Storage: Found invalid marker (0x{:02X}). Erasing storage...",
                val
            );
        ...
        Err(ref e) => {
            defmt::error!(
                "Storage: Error reading marker ({:?}). Erasing storage...",
    ...
    V: Serialize + MaxSize + Clone + DeserializeOwned,
{
    ...
    if required_buf_size_estimate > DATA_BUFFER_SIZE {
        defmt::error!(
            "Storage insert failed for key '{}': Estimated buffer size {} exceeds allocated buffer {}",
            key,
            required_buf_size_estimate,
            DATA_BUFFER_SIZE
        );
    ...
    V: DeserializeOwned + Serialize,
{
    ...
    match fetch_result {
        ...
        // Use 'ref _e @' because e is unused, use 'ref flash_err' to borrow inner value
        Err(
            ref _e @ Error::Storage {
                value: ref flash_err,
            },
        ) => {
            defmt::error!(
                "Storage error during fetch for key '{}': {:?}",
                key,
                defmt::Debug2Format(flash_err)
            );
            // Reconstruct error using copied flash_err (FlashError is Copy)
            Err(Error::Storage { value: *flash_err })
        }
        Err(e @ Error::BufferTooSmall(size)) => {
            defmt::error!(
                "Buffer too small (size {}) during fetch for key '{}'",
                size,
                key
            );
            Err(e)
        ...
        // Use 'ref _e @' and 'ref _inner...' to borrow inner value without moving
        Err(ref _e @ Error::SerializationError(ref _inner_serialization_err)) => {
            defmt::error!(
                "Unexpected serialization error during fetch for key '{}'",
                key
            );
            Err(Error::Corrupted {})
        }
...
pub fn erase_all() -> Result<(), Error<FlashError>> {
    ...
    let erase_result = unsafe {
        storage_mutex.lock_mut(|state| {
            ...
            state.cache = sequential_storage::cache::KeyPointerCache::<
                PAGE_COUNT,
                CacheKeyType,
                CACHE_KEYS,
            >::new();
            ...
            result.map_err(|flash_err| Error::Storage { value: flash_err }) // Correct syntax
        })
    };
    ...
    if erase_result.is_ok() {
        ...
        insert(MARKER_KEY, &MARKER_VALUE).map_err(|e| {
            defmt::error!(
                "Storage: FAILED to write marker after erase: {:?}",
                defmt::Debug2Format(&e)
            );
            e
        })?;
...
pub fn remove(key: &str) -> Result<(), Error<FlashError>> {
    ...
    defmt::warn!(
        "Storage: remove() called for key '{}', but is currently disabled for this target due to potential performance/driver limitations.",
        key
    );
    Ok(())
}
...
/*
// remove() implementation if enabled
pub fn remove(key: &str) -> Result<(), Error<FlashError>> {
    let padded_key = pad_key(key).ok_or_else(|| Error::BufferTooSmall(CACHE_KEY_BUFFER_SIZE))?;
